...
const InterviewWithMentor = (props) => {
    ...
    useEffect(() => {
        ...
        let domains = [];
        let roles = [];
        fetch('http://localhost:8080/interviewWithMentor').then(response => response.json()).then(data => {
            setMentorList(data);
    ...
    return (
        <div className="pt-2 pl-5 pr-5 pb-2" id="interview-with-mentor" >
            {props.isAuthed ? <NavbarBrand setAuth={props.setAuth} /> : <NavbarBeforeSignUp page='/interviewWithMentor' />}
            ...
            <div className="row">
                ...
                {(filterdMentorList && filterdMentorList.length > 0) ? <div className="col-10 " style={{ "textAlign": "center" }}>
                ...
                </div> : <div>No mentor found, <Button style={{ "minWidth": "180px" }} id="btn-practice" onClick={() => { setFilterdMentorList(mentorList); setFilteredCompany(undefined); setFilteredDomain(undefined); setFilteredRole(undefined); }}>Clear Filter</Button></div>}
            </div>
        </div>
    )
}
