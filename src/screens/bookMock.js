...
const BookMock = (props) => {
  const checkAuthenticated = async () => {
    try {
      const res = await fetch("http://localhost:8080/authentication/verify", {
        method: "POST",
  ...
  const sendToBackend = () => {
    ...
    fetch('http://localhost:8080/interviewWithMentor/generatemeetlink', {
      method: "POST", headers: {
  ...
  const onChange = (value) => {
    ...
    let dateString = year + "-" + month + "-" + day;
    fetch('http://localhost:8080/interviewWithMentor/get_timing', {
      method: "POST", headers: {
        "Content-type": "application/json"
      },
      body: JSON.stringify({
